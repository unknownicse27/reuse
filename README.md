...
We also present the results of ReuSE and the baselines for the main experiment.

Also, in each test directory, you will see the following files:
+ bug_reports : Bug-finding results for each experiment.
+ testcases : Test cases generated by each experiment.
  + iteration-{ver}-* : Iterations for each releases.
     ...
     + test*.const : Path conditions accumulated during test case generation.
