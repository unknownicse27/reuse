...
KleeHandler::KleeHandler(int argc, char **argv)
  ...
  // [ADDED] Export output dir so libraries (e.g., CachingSolver) can write alongside results
#if defined(_WIN32)
  _putenv_s("KLEE_OUTPUT_DIR", std::string(m_outputDirectory.c_str()).c_str());
#else
  // overwrite=1 to ensure the latest run’s directory is visible to solver library
  ::setenv("KLEE_OUTPUT_DIR", m_outputDirectory.c_str(), /*overwrite*/1);
#endif
