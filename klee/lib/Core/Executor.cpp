...
Executor::StatePair Executor::fork(ExecutionState &current, ref<Expr> condition,
  ...
  if (res==Solver::True) {
    ...
    TimerStatIncrementer timer(stats::forkTime);
    // KLEE invariant: right child must be the current state.
    // Keep role:
    //   true  branch = reuse parent (current)
    //   false branch = create new child
    ...
    // attach(left=false child, right=current true child)
    executionTree->attach(current.executionTreeNode, falseState, trueState, reason);
    stats::incBranchStat(reason, 1);

    if (pathWriter) {
      // falseState is the new child: open its stream from the parent
      falseState->pathOS = pathWriter->open(current.pathOS);
      if (!isInternal) {
        trueState->pathOS  << "1"; // right/current is '1'
        falseState->pathOS << "0"; // left/new child is '0'
      }
    }
    if (symPathWriter) {
...
bool Executor::getSymbolicSolution(const ExecutionState &state,
  ...
  std::string ktestPath = interpreterHandler->getOutputFilename("test" + testId + ".ktest");
  std::string outDir = ktestPath.substr(0, ktestPath.find_last_of('/'));
  ...
  if (constraintFile.is_open()) {
    ...
    bool first = true;
    for (auto it = state.forkedConstraints.begin(); it != state.forkedConstraints.end(); ++it) {
      std::string buf;
      llvm::raw_string_ostream rso(buf);
      (*it)->print(rso);
      rso.flush();
      buf.erase(std::remove(buf.begin(), buf.end(), '\n'), buf.end()); // Remove line breaks
      ...
      for (char c : buf) {
        if (isspace(c)) {
          ...
          // Handle characters that need escaping inside a JSON string
          if (c == '"') compact += '\\';
          compact += c;
          prevSpace = false;
        }
      }
      if (!first) constraintFile << ", "; // Handle commas
      first = false;
      constraintFile << "\"" << compact << "\""; // Output as a JSON-safe string
    }
    constraintFile << "]\n";
    constraintFile.close();
    llvm::outs() << "[KLEE] Constraints written to: " << filename << "\n";
  } else {
    llvm::errs() << "[KLEE] Failed to write constraint file: " << filename << "\n";
  }
  return true;
}
