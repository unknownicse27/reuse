...
SpecialFunctionHandler::readStringAtAddress(ExecutionState &state, 
  ...
  char c = 0;
  bool warnedSymbolicByte = false;
  bool stoppedBySymbolicByte = false;
  for (size_t i = offset; i < mo->size; ++i) {
    ref<Expr> cur = os->read8(i);
    cur = executor.toUnique(state, cur);
    if (!isa<ConstantExpr>(cur)) {
      if (!warnedSymbolicByte) {
        klee_warning_once(0,
          "Symbolic byte encountered while reading string for klee_* API; "
          "using truncated/fallback string");
        warnedSymbolicByte = true;
      }
      stoppedBySymbolicByte = true;
      break;
    }
    c = (char)cast<ConstantExpr>(cur)->getZExtValue(8);
    if (c == '\0') {
...
void SpecialFunctionHandler::handleMakeSymbolic(ExecutionState &state,
  ...
  if (arguments[2]->isZero()) {
    name = "";
  } else {
    ref<Expr> namePtr = executor.toUnique(state, arguments[2]);
    if (!isa<ConstantExpr>(namePtr)) {
      klee_warning_once(0, "klee_make_symbolic(): symbolic name pointer; using fallback name");
      name = "";
    } else {
      name = readStringAtAddress(state, namePtr);
    }
  }

  if (name.length() == 0) {
    ref<Expr> a = executor.toUnique(state, arguments[0]);
    ref<Expr> n = executor.toUnique(state, arguments[1]);
    uint64_t ap = isa<ConstantExpr>(a) ? cast<ConstantExpr>(a)->getZExtValue() : 0;
    uint64_t sz = isa<ConstantExpr>(n) ? cast<ConstantExpr>(n)->getZExtValue() : 0;
    name = "unnamed_" + llvm::utohexstr(ap) + "_" + std::to_string(sz);
    klee_warning_once(0, "klee_make_symbolic: empty/unknown name; using \"%s\"", name.c_str());
  }
